...
class Data:
    ...
    **Attributes**
    ...
    def set_data(self, file: pd.DataFrame):
...
class CSV:
    """Data Structure to wrap all operations needed to parse .csv files into Python.

    **Attributes**

    - **data:** (Data Object) Stores Pandas DataFrame with variables values according csv file
    - **df:** (pd.DataFrame Object) stores tabular data of variable according csv file
    """
    ...
    def set_data(self, file: pd.DataFrame):
        """Parses tabular time series data of a .csv file into a Pandas DataFrame
    ...
    def read(self, filepath: str, **kwargs):
        ...
        **Parameters**
        ...
        if ext == ".csv":
            ...
            df = pd.read_csv(filepath + ".csv", **kwargs)
        
        self.data.set_data(file=df)

    @property
    def df(self) -> pd.DataFrame:
        """Gets tabular time series data of a .csv file
...
class CSVFormatter:
    r"""
        Wraps all operations needed to handle comma-separated values (csv) files.
    """
    def __init__(self):
        self.__file = namedtuple("File", "csv")
        self.__extensions = ['.csv']
    ...
    def read_csv(self, filename:str)->CSV:
        """
        Reads .csv file given its filename
    ...
    def read(self, filepath, **kwargs)->GeneratorType:
        """
        Reads csv files into a CSV Object Structure

        **Parameters**

        - **filepath:** (str) filename with/without csv exntesion (.csv) or foldername where are all csv files

        **Returns**

        - **object:** (GeneratorType) with 'csv' attribute
        """
        
        if os.path.isdir(filepath):

            files = self.read_folder(filepath, **kwargs)
            ...
            yield self.read_file(filepath, **kwargs)

    def read_file(self, filepath, **kwargs)->pd.DataFrame:
        """
        ...
        """   
        csv = CSV()
        csv.read(filepath, **kwargs)
        return self.group_csv_files(csv)

    def read_folder(self, path:str, **kwargs):
        """
            Reads all csv file located in a path

            **Parameters**

            - **path:** (str) path where CSV files are.
            
            **Returns**

            - **object:** (GeneratorType)
        """
        files = Helpers.get_files(filepath=path, ext=".csv")
            
        for file in files:
            
            yield self.read_file(file, **kwargs)

    def group_csv_files(self, csv: CSV)->namedtuple:
        """
            Groups CSV files into an structure
        
            **Parameters**

            - **csv:** (CSV) A CSV Object

            **Returns**

            - **object:** (namedtuple) With object.dataframe
        """
        if isinstance(csv, CSV):

            file = self.__file(csv)
            return file